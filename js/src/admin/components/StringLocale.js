...
export default class StringLocale {
  oninit(vnode) {
    ...
    this.string = app.store.all('fof-linguist-string').find((string) => string.key() === this.stringKey.key() && string.locale() === this.localeKey);
  ...
  view(vnode) {
    const placeholderText = this.originalString
      ? this.originalString
      : '(' + app.translator.trans('fof-linguist.admin.placeholder.' + (this.localeKey ? 'not-translated' : 'all-locales')) + ')';
