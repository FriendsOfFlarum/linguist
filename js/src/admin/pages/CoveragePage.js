...
export default class CoveragePage {
      ...
      m('table.FoF-Linguist-Coverage', [
        m(
          ...
          m('tr', [
            ...
            this.columns === 'locale'
              ? localesAsArray().map((locale) => m('th', locale.name + ' (' + locale.key + ')'))
              : [
                  this.showAllFrontends
                    ? visibleFrontends.map((frontend) => m('th', frontendLabel(frontend)))
                    : [
        ...
        m('tbody', [
          ...
          vnode.attrs.namespaces.map((namespace) => {
            return m('tr', [
              ...
              this.columns === 'locale'
                ? localesAsArray().map((locale) => m('td', this.localeCoverage(browseWithFilters, locale.key, namespace.namespace)))
                : [
                    this.showAllFrontends
  ...
  localeCoverage(browseWithFilters, locale, namespace, frontend) {
    ...
    stringKeys.forEach((stringKey) => {
      const customString = app.store.all('fof-linguist-string').find((string) => string.key() === stringKey.key() && string.locale() === locale);
      ...
      m(
        ...
        [
          ...
          m(
            ...
            app.translator.trans('fof-linguist.admin.coverage.count.details', {
              total: stringKeys.length,
